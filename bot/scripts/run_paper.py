...
from bot.adapters.paper import PaperAdapter, PaperConfig
...
def main() -> None:
    ...
    for symbol in symbols:
        ...
        allow_short = bool(symbol.params.get("allow_short", False))
        symbol_to_adapter[symbol.symbol] = PaperAdapter(base_adapter, PaperConfig(allow_short=allow_short))

    adapter = MultiVenueAdapter(symbol_to_adapter)
    Path("bot/state").mkdir(parents=True, exist_ok=True)
    persistence = Persistence("bot/state/paper.db")
    ...
    logger.info(event="paper_start", details={"symbols": [s.symbol for s in symbols]})
    ...
    while True:
        for symbol in symbols:
            try:
                ...
                alerter.send(f"Paper cycle error for {symbol.symbol}: {exc}")
